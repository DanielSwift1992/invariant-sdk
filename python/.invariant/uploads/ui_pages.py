        ...
        document.addEventListener('click', (e) => {
            ...
            content.innerHTML = '<div class="loading"><span class="spinner"></span>Searching...</div>';
		        ...
		        function renderResults(data) {
		            ...
		            // File card click -> navigate to file page
		            document.querySelectorAll('.file-card').forEach(card => {
		                card.style.cursor = 'pointer';
		                card.addEventListener('click', (e) => {
		                    // Don't navigate if clicking on buttons/links/details inside
		                    if (e.target.closest('button, a, details, .occ-line')) return;
		                    const doc = String(card.dataset.doc || '');
		                    if (!doc) return;
		                    const q = queryInput.value.trim();
		                    const href = '/doc?doc=' + encodeURIComponent(doc) + (q ? ('&q=' + encodeURIComponent(q)) : '');
